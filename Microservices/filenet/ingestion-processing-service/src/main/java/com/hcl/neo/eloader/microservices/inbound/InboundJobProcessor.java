...
public class InboundJobProcessor extends JobProcessor {
	...
	public void process(BulkJobParams params) throws BulkException {
		try {
			...
			jobProcessService.addJobStatus(params.getId(), "Archive "+BulkJobStatus.CREATED.toString());
			...
			jobProcessService.addJobStatus(params.getId(), BulkJobStatus.IN_PROGRESS_TRANSPORTER.toString());
			download(params);
			...
			jobProcessService.addJobStatus(params.getId(), BulkJobStatus.IN_PROGRESS_ARCHIVER.toString());
			...
			if (validated) {
				...
				jobProcessService.addJobStatus(params.getId(), BulkJobStatus.IN_PROGRESS_REPO.toString());
	...
	public void updateJobStatus(Long jobId, JsonElement operationObjectDetails){
		...
		try {
			...
			updateJobDetails(jobId, new Date(), objectDetailList);
			jobProcessService.addJobStatus(jobId, getJobStatusFromResponse(objectDetailList).toString());		
		} catch (Throwable e) {
			e.printStackTrace();
		}
	}
	...
	private boolean download(BulkJobParams params) throws EloaderDbException, TransporterException {
		...
		System.out.println("Inside download"+params.getId());
		if (record != null) {
	...
	private Response storeInRepository(BulkJobParams params) throws RepositoryException {
		...
		try {
			...
			if (BulkJobType.IMPORT.equals(params.getType()) || BulkJobType.INGESTION.equals(params.getType())) {

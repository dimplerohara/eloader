...
public class JMSProducer {
	...
	public void sendImportWrapperMsg(final BulkJobParams params) throws Throwable {
		...
		if (BulkJobType.IMPORT.equals(params.getType()) || BulkJobType.INGESTION.equals(params.getType())) {
			...
			System.out.println(JsonApi.toJson(importContentParams));
			jobProcessService.addJobStatus(params.getId(), "QUEUED for Wrapper Service");
			if (size > minSize) {
			...
			ServiceLogger.info(this, " - sendBulkJobMsg() ");
				...
				|| BulkJobType.EXPORT.equals(params.getType())
				|| BulkJobType.CHECKOUT.equals(params.getType())){
	...
	private boolean createLightJobQueue(final String messageText){
		System.out.println("Inside createLightJobQueue block");
		boolean flag = true;
		try{
	...
	private ImportContentParams toImportContentParams(ImportParams operationParams){
		...
		importContentParams.setRepoType(operationParams.getRepoType());
	...
	private ExportContentParams toExportMetadataParams(ExportMetadataParams params){
		...
		exportContentParams.setJobType(params.getJobType());

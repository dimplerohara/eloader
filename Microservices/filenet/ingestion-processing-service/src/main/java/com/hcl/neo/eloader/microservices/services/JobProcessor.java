...
public abstract class JobProcessor {
  ...
  /*  @Autowired
        ...
        try {
           ...
           // List<OperationObjectDetail> objectDetailList = response.getOperationObjectDetails();
            if(null != objectDetailList){
            	...
            	Logger.info(getClass(), "Success Count is---> "+noOfSuccess+" and Failure Count is---> "+noOfFailures);
            	updateJobSucessCount(jobId,noOfSuccess,noOfFailures);               
            }else{
            	Logger.info(getClass(), "ObjectDetailList is null...");
            }
        } catch (Throwable e) {
    ...
    protected void updateJobSucessCount(Long jobId,int successCount,int failureCount) throws EloaderDbException {
        JobMaster record = jobMasterRepository.findByJobId(jobId);
        Logger.info(getClass(), "record found-->"+record);
    ...
    protected void updateJobDetails(Long jobId, Date date, List<OperationObjectDetail> objectDetailList) throws EloaderDbException {
        JobMaster record = jobMasterRepository.findByJobId(jobId);
        record.setCompletionDate(date);
        jobMasterRepository.save(record);
    }
    
    protected void updateCompletionDate(Long jobId, Date date) throws EloaderDbException {
        JobMaster record = jobMasterRepository.findByJobId(jobId);
        record.setCompletionDate(date);
        jobMasterRepository.save(record);
    }

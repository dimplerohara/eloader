...
public class JmsConsumer {
	...
	@Value("${eloader.filenet.uri}")
	String filenetURI;

	@Value("${eloader.filenet.stanza}")
	String filenetStanza;
	...
	private void callServiceWithHeader(String message, String url){
		try{
			...
			if(!(filenetURI==null || filenetURI.equals(""))){
				headers.set("URI", filenetURI);
			}
			if(!(filenetStanza==null || filenetStanza.equals(""))){
				headers.set("STANZA", filenetStanza);
			}
			HttpEntity<String> entity = new HttpEntity<String>(message,headers);
	...
	public void receiveHeavyWrapperJob(final Message message){
		...
		try {
			...
			if(type.equalsIgnoreCase("IMPORT") || type.equalsIgnoreCase("INGESTION") ){
			...
			}else if(type.equalsIgnoreCase("EXPORT") || type.equalsIgnoreCase("CHECKOUT")){

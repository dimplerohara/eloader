...
/**
 * Helper Class for import Metadata opeartion
 * @author sakshi_ja
 */
@Component
public class ImportMetadataOperationHelper {
	...
	/**
	 * Method to process objects and calling DAO method for importing metadata for objects.
	 * @param objects
	 * @param filenetDao
	 * @param strUserName
	 ...
	 * @return
	 */
	public List<OperationObjectDetail> importMetadata(Objects objects, CmsDao filenetDao,String strUserName,String objectStoreName) {
		...
		for (com.hcl.neo.cms.microservices.excel.schema_metadata.Object object : objects.getObject()) {
			...
			try 
			{
				...
				IndependentObject idObject=filenetDao.getObjectByIdentity(identity,objectStoreName);
				if(idObject instanceof Document){	
					Logger.info(getClass(), "Document Object Found");
					docObject=(Document)idObject;
				}else if(idObject instanceof Folder){

					Logger.info(getClass(), "Folder Object Found");
				...
				if (docObject != null) {
					...
					if (folderObject != null) {
						...
						if (isRequiredPermission) {
							isObjectUpdated=filenetDao.updateObjectProps(getUpdateObjectParam(object,filenetDao,folderObject,identity),objectStoreName);
							if (isObjectUpdated == true) {
